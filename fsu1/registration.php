	...
	elseif(mysqli_num_rows($check_email) > 0) //check email
     {
        ...
        $safe_username = $db->real_escape_string($_POST['username']);
        $safe_firstname = $db->real_escape_string($_POST['firstname']);
        $safe_lastname = $db->real_escape_string($_POST['lastname']);
        $safe_phone = $db->real_escape_string($_POST['phone']);
        $safe_email = $db->real_escape_string($_POST['email']);
        $safe_password = $db->real_escape_string($_POST['password']);
        $safe_address = $db->real_escape_string($_POST['address']);

        $hashed_password = md5($safe_password);
       
	 //inserting values into db
	$mql = "INSERT INTO users(username,f_name,l_name,email,phone,password,address) VALUES('$safe_username','$safe_firstname','$safe_lastname','$safe_email','$safe_phone','$hashed_password','$safe_address')";
	mysqli_query($db, $mql);
